# Databricks notebook source exported at Mon, 10 Oct 2016 20:33:24 UTC
...
print 'Ratings:'
ratings_df.show(3)
test1 = ratings_df.show(3)
print 'Movies:'
movies_df.show(3, truncate=False)
test2 = movies_df.show(3, truncate=False)
...
#testrdd=movies_df.join(ratings_df)
#display(testrdd)
...
#display(movies_df.withColumnRenamed('ID', 'movieId'))
#display(movies_df.withColumnRenamed('ID', 'movieId').join(movie_ids_with_avg_ratings_df, "movieId"))

### once DF.map then ==> RDD and cannot use RDD.show()
movie_names_with_avg_ratings_df = (movies_df.withColumnRenamed('ID', 'movieId').join(movie_ids_with_avg_ratings_df,"movieId") )

                  #.map(lambda (movieId, (title, (count, average))): (average, title, count)).toDF(["average","title","count"])) 

#movie_names_df = (movies_df.withColumnRenamed('ID', 'movieId').join(movie_ids_with_avg_ratings_df,"movieId").select(movie_names_df.average , movie_names_df.title, movie_names_df.count , movie_names_df.movieId )                  
...
seed = 1800009193L
(split_60_df, split_a_20_df, split_b_20_df) = ratings_df.randomSplit([0.6, 0.2, 0.2], seed = 1800009193L)
...
validationForPredictRDD = validation_df.rdd.map(lambda (userId, movieId, rating): (userId, movieId))
...
for rank in ranks:
  ...
  ## predict_df = model.predictAll(validation_df)
  
  ##predictedRatingsRDD = model.predictAll(validationForPredictRDD)
  ##predict_df = predictedRatingsRDD.toDF()
...
# TODO: Replace <FILL_IN> with the appropriate code.
#training_df.show(30)
#validation_df.show(30)
#test_df.show(30)

# Compute the average rating
avg_rating_df = training_df.agg(F.avg(training_df.rating))  #training_df.groupBy('userId','movieId').mean()  #groupBy('movieId').agg(F.count(ratings_df.rating).alias("count"), F.avg(ratings_df.rating).alias("average"))
#display(avg_rating_df)
display(avg_rating_df)

# Extract the average rating value. (This is row 0, column 0.)
training_avg_rating = avg_rating_df.collect()[0][0]
print (training_avg_rating)
...
#display(movies_with_500_ratings_or_more.filter(movies_with_500_ratings_or_more['average']<3).take(40) )
...
als.setPredictionCol("prediction")\
   .setMaxIter(5)\
   .setSeed(seed)\
   .setSeed(seed).setRegParam(0.1).setUserCol("userId").setItemCol("movieId").setRatingCol("rating").setRank(ranks[best_rank])
...
predicted_highest_rated_movies_df = predicted_with_counts_df.orderBy(F.desc("prediction"))

print ('My 25 highest rated movies as predicted (for movies with more than 75 reviews):')
predicted_highest_rated_movies_df.filter(predicted_highest_rated_movies_df['count']>75).show(25)
